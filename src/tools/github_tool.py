...
"""GitHub automation tool - FIXED with proper timeout handling"""
import os
import subprocess
from github import Github, GithubException
from typing import Dict, Any, Optional
...
class GitHubTool:
    ...
    async def push_local_code(
        ...
        """Push local code to GitHub repository with FIXED timeout handling"""
        try:
            ...
            if not git_dir.exists():
                ...
                if returncode != 0:
                    ...
                    return {"success": False, "error": f"Git init failed: {stderr}"}
            ...
            for cmd in commands:
                returncode, stdout, stderr = self._run_git_command(cmd, str(local_path), timeout=10)
                if returncode != 0:
                    logger.warning(f"Config command failed (continuing anyway): {stderr}")
            ...
            # Add remote
            logger.info(f"Adding remote: {repo.clone_url}")
            returncode, stdout, stderr = self._run_git_command(
                ["git", "remote", "add", "origin", repo.clone_url],
                str(local_path),
                timeout=10
            ...
            if returncode != 0:
                ...
                return {"success": False, "error": f"Failed to add remote: {stderr}"}
            ...
            # Push with LONGER timeout (5 minutes for large files)
            logger.info(f"Pushing to GitHub (this may take a while for large projects)...")
            returncode, stdout, stderr = self._run_git_command(
                ["git", "push", "-u", "origin", branch, "--force"],
                str(local_path),
                timeout=300  # 5 minutes timeout instead of 30 seconds
            )
            ...
            if returncode != 0:
                ...
                return {
                    ...
                    "data": {
                        "repo_url": repo.html_url,
                        "suggestion": "Files committed locally. Try pushing manually with: cd {} && git push -u origin {}".format(local_path, branch)
                    }
                }
    ...
    async def build_and_push_project(
        ...
        try:
            ...
            logger.info(f"Step 1: Creating GitHub repository '{repo_name}'")
            create_result = await self.create_repo(repo_name, description)
            ...
            # Step 2: Push code
            logger.info(f"Step 2: Pushing code to repository")
            ...
            return {
                "success": True,
                "message": f"Project '{repo_name}' created and code pushed successfully",
                "data": {
...
This project was automatically created from a Figma design using MCP Automation.
...
[MCP Automation](https://github.com/your-username/mcp-automation) - AI-powered automation tool
"""
